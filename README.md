...
```
+-----------------------------------------------------------------------------------+
|                           CLIENT BROWSER (Micro-Frontends)                        |
|                                                                                   |
|  +------------------------+      +--------------------+    +--------------------+ |
|  |       HOST APP         | <--> |   DASHBOARD MFE    |    |    TRACKING MFE    | |
|  |     (React Shell)      |      |   (Order Stats)    |    |     (Live Map)     | |
|  +-----------+------------+      +--------------------+    +--------------------+ |
|              | HTTP / WSS                                                         |
+--------------|--------------------------------------------------------------------+
               |
...
|          (Auth, Aggregation, Socket.io Server)            |       | (Socket Store)|
+---+---------------------------^-----------------------^---+       +---------------+
    | HTTP                      | AMQP                  | TCP
    |                           |                       |
    |                 +---------+-----------+   +-------+---------+
    |                 |      RABBITMQ       |   |      KAFKA      |
    |                 |   (Notifications)   |   | (Location Data) |
    |                 +---------^-----------+   +-------^---------+
    |                           |                       |
    |                           | Pub "order_created"   | Pub "driver-moved"
    v                           |                       |
+-------------------+     +-----+-------------+    +----+--------------+
|   ORDER SERVICE   |     |   ORDER SERVICE   |    |  TRACKING SERVICE |
|     (Node.js)     |     |   (Worker/Logic)  |    |     (Node.js)     |
+---------+---------+     +-------------------+    +-------------------+
          |
          | HTTP (Trigger)
          v
